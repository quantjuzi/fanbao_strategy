...
我有7年A股短线交易经验。这个策略最早是我在问财上一个条件一个条件手工筛选的，后来学了 Python，把手工流程一步步变成了代码。这个仓库记录了我从手工交易到量化回测的完整过程。
...
- 切换至东方财富账户，此前银河证券开盘卡顿影响操作
- 7月中旬后，反包信号减少。选股从单一反包模式扩展为多维度筛选，结合分时承接和板块强度综合判断
- 市场题材轮动加快，不再固定于单一题材方向
...
- 卖出点：根据当天同板块题材强弱、大盘情况、热门股赚钱效应灵活调整
...
回测区间：2026-04-12 至 2026-06-12，共 2 个月。因CSV文件大小限制，回测按2个月一段分段进行，测试代码已放到 backtest/ 目录下，大家可以自取。篇幅有限，这里只展示最近两个月的回测数据。
...
> **说明**：以上回测结果按开盘价买入、收盘价卖出计算，未扣除手续费（当前市场佣金普遍万1左右）。买入标的均为大票，流动性充足，几乎不产生滑点。因此回测数据与实盘收益偏差不大。
...
每日选股逻辑、候选股多维对比分析及交易执行记录详见 [daily_notes/](./daily_notes/) 目录。
...
| [02_因子对比分析](./教程/02_因子对比分析.md) | 如何理解回测数据、调优因子——附完整实战案例 |
