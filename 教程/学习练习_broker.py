# 学习练习：逐行说出 broker.py 每行在干嘛
# 用法：
# 1. 从上往下读，每行在脑子里翻译成一句中文
#    例：self.config = config or BrokerConfig()
#        → 如果没传配置，就用默认的 BrokerConfig()
# 2. 读完全部 8 个 TODO，先只补一个 sell_fill_price 试试
# 3. 检查方式：在项目根目录运行 python tests\test_broker.py
# 注意：不要背代码，只看每一行"在做什么交易操作"
